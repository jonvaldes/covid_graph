...
fn get_per_country_data() -> Result<Vec<RegionData>> {
    ...
    for data_point in get_covid_data()? {
        ...
        if let Some(value) = hash.get_mut(&data_point.name) {
            ...
            hash.insert(data_point.name.clone(), RegionData{
...
fn draw_evolution_graph(regions: &Vec<RegionData>) -> Result<()>{
    let interesting_countries = &["Spain", "Sweden", "Norway", "Italy", "France", "Germany", "Belgium", "Finland", "Austria", "Netherlands", "Switzerland"];
    ...
    let (upper, lower) = root_area.split_vertically(512);
    {
        let mut cc = ChartBuilder::on(&upper)
            ...
            .build_ranged(0..24*5*30i64, 0..40_000i64)?;

        cc.configure_mesh()
            .x_labels(20)
            .y_labels(10)
            .disable_mesh()
            .x_label_formatter(&|v| format!("{:.1}", v/24))
            .y_label_formatter(&|v| format!("{:.1}", v))
            .draw()?;

        for (index, country) in regions.iter().filter(|x| interesting_countries.contains(&x.name.as_str())).enumerate()
        {
            let t0 = Utc.ymd(2020,1,22).and_hms(0,0,0);

            cc.draw_series(LineSeries::new(
                country.evolution.iter().map(|point| 
                    (point.update_time.signed_duration_since(t0).num_hours() as i64,
                    point.deaths as i64)),
                    &GraphPalette::pick(index),
            ))?
                .label(country.name.clone())
                .legend(move |(x, y)| PathElement::new(vec![(x, y), (x + 20, y)], &GraphPalette::pick(index)));
        }
    ...
    }
    {
        ...
        let mut cc = ChartBuilder::on(&lower)
            .margin(5)
            .set_all_label_area_size(50)
            .caption("Deaths per 100K", ("sans-serif", 40).into_font())
            .build_ranged(0f32..(24*5*30) as f32, 0f32..80f32)?;

        cc.configure_mesh()
            .x_labels(20)
            .y_labels(10)
            .disable_mesh()
            .x_label_formatter(&|v| format!("{:.1}", v/24.0))
            .y_label_formatter(&|v| format!("{:.1}", v))
            .draw()?;
        ...
        for (index, country) in regions.iter().filter(|x| interesting_countries.contains(&x.name.as_str())).enumerate()
        {
            ...
            let population_over100k = population_map.get(&country.name).cloned().unwrap() as f32 / 100_000 as f32;
